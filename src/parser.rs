...
use crate::util::get_ssl_connection;

pub type RequestFuture =
    Pin<Box<dyn Future<Output = Result<Request<IncomingBody>, hyper::Error>> + Send>>;
...
{
    let stream = get_ssl_connection(addr, &sni).await.unwrap();
    http_request(req, stream, before, after).await
}
